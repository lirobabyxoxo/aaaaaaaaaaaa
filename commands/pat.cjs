...
module.exports = {
    ...
    async execute(message, args, client, config, colors, createYakuzaEmbed) {
        ...
        await performRoleplay('pat', message.author, target, message, null, config, colors, createYakuzaEmbed);
    },
    
    async executeSlash(interaction, client, config, colors, createYakuzaEmbed) {
        const target = interaction.options.getUser('usuário');
        await performRoleplay('pat', interaction.user, target, null, interaction, config, colors, createYakuzaEmbed);
    }
};
