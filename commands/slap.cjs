...
module.exports = {
    ...
    async execute(message, args, client, config, colors, createYakuzaEmbed) {
        ...
        await performRoleplay('slap', message.author, target, message, null, config, colors, createYakuzaEmbed);
    },
    
    async executeSlash(interaction, client, config, colors, createYakuzaEmbed) {
        const target = interaction.options.getUser('usuário');
        await performRoleplay('slap', interaction.user, target, null, interaction, config, colors, createYakuzaEmbed);
    }
};
